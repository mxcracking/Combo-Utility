...
export const captureRemover = (text: string): string => {
  // Simple and direct approach like the original JavaScript
...
export const applyOutputFormat = (text: string, format: OutputFormat): string => {
  ...
  switch (format) {
    ...
    case 'both':
      // Return both formats - keep email:pass AND add user:pass versions
      const emailPassLines = lines.filter(line => line.includes('@'));
      const userPassLines = lines.map(line => {
        if (line.includes('@')) {
          // Convert email:pass to user:pass
          return emailToUser(line);
        }
        return line;
      }).filter(line => line.trim());
      
      // Combine: first all email:pass lines, then all user:pass lines
